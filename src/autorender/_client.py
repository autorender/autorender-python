...
from ._exceptions import APIStatusError, AutorenderError
...
class Autorender(SyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous Autorender client instance.

        This automatically infers the `api_key` argument from the `AUTORENDER_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("AUTORENDER_API_KEY")
        if api_key is None:
            raise AutorenderError(
                "The api_key client option must be set either by passing api_key to the client or by setting the AUTORENDER_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("AUTORENDER_BASE_URL")
        if base_url is None:
            base_url = f"https://upload.autorender.io"
    ...
    def with_raw_response(self) -> AutorenderWithRawResponse:
        return AutorenderWithRawResponse(self)

    @cached_property
    def with_streaming_response(self) -> AutorenderWithStreamedResponse:
        return AutorenderWithStreamedResponse(self)
...
class AsyncAutorender(AsyncAPIClient):
    # client options
    api_key: str
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncAutorender client instance.
    ...
    @cached_property
    def with_streaming_response(self) -> AsyncAutorenderWithStreamedResponse:
        return AsyncAutorenderWithStreamedResponse(self)
...
class AutorenderWithRawResponse:
    _client: Autorender

    def __init__(self, client: Autorender) -> None:
        self._client = client
...
class AsyncAutorenderWithRawResponse:
    ...
    def __init__(self, client: AsyncAutorender) -> None:
        self._client = client
